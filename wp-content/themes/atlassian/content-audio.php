...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<?php atlassian_post_thumbnail(); ?>

	<header class="entry-header">
		<?php if ( in_array( 'category', get_object_taxonomies( get_post_type() ) ) && atlassian_categorized_blog() ) : ?>
		<div class="entry-meta">
			<span class="cat-links"><?php echo get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'atlassian' ) ); ?></span>
		</div><!-- .entry-meta -->
		<?php
			endif;
		...
		<div class="entry-meta">
			...
			<?php edit_post_link( __( 'Edit', 'atlassian' ), '<span class="edit-link">', '</span>' ); ?>
		</div><!-- .entry-meta -->
	</header><!-- .entry-header -->
